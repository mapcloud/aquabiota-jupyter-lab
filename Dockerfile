...
ENV SHELL /bin/bash
ENV AQUABIOTA_GIT_DIR $HOME/git
...
# Anaconda3-4.4.0-Linux-x86_64.sh original
RUN cd $HOME
RUN wget --quiet https://repo.continuum.io/archive/Anaconda3-5.0.0-Linux-x86_64.sh -O $HOME/anaconda.sh && \
...
RUN conda install -y bcrypt passlib ujson luigi
